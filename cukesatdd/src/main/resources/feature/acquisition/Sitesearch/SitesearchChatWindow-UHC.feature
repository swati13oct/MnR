...
@ChatWindowBlayer
Scenario Outline: 1.20 To verify ChatWindow in all the pages of UHC site
Given the user is on uhcmedicaresolutions site landing page
Then the user validates whether chat icon on UHC site

  @VppChatWindowBlayer123 @ChatWindowBlayer 
...
#   Then the user hovers over the tool tip for Annual Deductible and validates the ext for PDP Plan in UMS Site  
 ...
 # |  15542 |  90210  | NO              | Los Angeles County | MA       | AARP Medicare Advantage SecureHorizons Essential (HMO) | $0              | $5  copay             | $10  copay  | Yes              |  $4,900.00         |  No drug coverage                            |                                                        | 
   #|  15543 |  90210  | NO              | Los Angeles County | PDP      | AARP MedicareRx Walgreens (PDP)                      | $0     
    ...
    @ComparePlansChatWindowBlayer @ChatWindowBlayer                                                                                                                                                                                                                                                             
...
@DCEChatWindowBlayer  @ChatWindowBlayer
  ...
  @OLEUHCChatWindowBlayer @ChatWindowBlayer
  Scenario Outline: TID: <TID> -plan type: <PlanType> - OLE End to end from UHC Acquisition site VPP Plan Summary with chat icon validation
    Given the user is on UHC medicaresolutions site landing page
    Then the user validates chat Icon on UHC site
    When the user performs plan search using following information in the UHC site
      | Zip Code        | <zipcode>         |
      | County Name     | <county>          |
    ...
    And the user validates the available plans for selected plan types in the UHC site
    Then the user validates chat Icon on UHC site
    Then the user clicks on Enroll Now for UHC site to start the OLE flow
      | Plan Name | <planName> |
    Then the user validates chat Icon on UHC site
