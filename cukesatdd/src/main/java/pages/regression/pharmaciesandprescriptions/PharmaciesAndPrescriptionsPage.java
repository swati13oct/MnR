...
public class PharmaciesAndPrescriptionsPage extends PharmaciesAndPrescriptionsBase {
	...
	public void clickOnMedicationName() {
		Assert.assertTrue("PROBLEM - unable to locate Medicine name element", pnpValidate(FirstCurrentMedication));
	...
	public void validateDrugInfopage() {
		...
		if (activeMedicineName.contains(MedicationName_OnDrugInfoPage.getText())) {
			...
			flag = false;
			Assert.assertTrue("PROBLEM -  User is NOT redirected to expected active drug info page.", flag);
		}
	...
	/*
	 * public boolean validateOrderStatusForHDDrug(String orderStatus) {
	 * List<Integer> listOfIndex =
	 * getOrderStatusIndexBasedOnStatusValue(orderStatus); if (listOfIndex.size() !=
	 * 0) { for (Integer val : listOfIndex) { if
	 * (listOfPharmacyName.get(val).getText().equals("OptumRx")) { return false; } }
	 * return true;
	 * 
	 * } else { return false; }
	 * 
