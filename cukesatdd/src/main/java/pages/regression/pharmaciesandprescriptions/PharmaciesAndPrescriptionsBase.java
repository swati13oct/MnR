...
public class PharmaciesAndPrescriptionsBase extends PharmaciesAndPrescriptionsWebElements {
	...
	/**
	 * Validate Need Help section content note: Tab structure is Suppressed if the
	 * information provided on both the tabs is exactly similar. note: So in this
	 * case we not show the P&P page in case of a standalone ship member note: and
	 * will show the P&P page without any Plan Tabs in case of a combo member note:
	 * COMBO user with SHIP plan will have SHIP's need help content
	 * 
	 * @param planType
	 * @param memberType
	 * @return
	...
	public String validateNeedHelpSection(String planType, String memberType) throws InterruptedException {
		...
		if (planType.equalsIgnoreCase("SHIP") || planType.toUpperCase().contains("MEDSUPP")) {
			...
			validateNeedHelpSectionContent(validateSection, needHelp_TechicalSupportSection,
					needHelp_TechicalSupport_img, needHelp_TechicalSupport_phone, needHelp_TechicalSupport_tty,
					needHelp_TechicalSupport_wkDayHrs, needHelp_TechicalSupport_wkEndHrs);
			...
			validateNeedHelpSectionContent(validateSection, needHelp_GeneralQuestionsSection,
					needHelp_GeneralQuestions_img, needHelp_GeneralQuestions_phone, needHelp_GeneralQuestions_tty,
					needHelp_GeneralQuestions_wkDayHrs, needHelp_GeneralQuestions_wkEndHrs);

			validateSection = "Need Help - Claims Support";
			validateNeedHelpSectionContent(validateSection, needHelp_ClaimsSupportSection, needHelp_ClaimsSupport_img,
					needHelp_ClaimsSupport_phone, needHelp_ClaimsSupport_tty, needHelp_ClaimsSupport_wkDayHrs,
			...
			System.out.println(
					"New window URL = " + driver.getCurrentUrl() + "| New window title = " + driver.getTitle());
			Assert.assertTrue(
					"PROBLEM - not getting expected contact us URL. " + "Expected to contains='" + expContactUsUrl
							+ "' | Actual URL='" + driver.getCurrentUrl() + "'",
					driver.getCurrentUrl().contains(expContactUsUrl));
			Assert.assertTrue(
					"PROBLEM - not getting expected contact us Title. " + "Expected to contains='" + expContactUsTitle
							+ "' | Actual URL='" + driver.getTitle() + "'",
			...
			validateNeedHelpSectionContent(validateSection, needHelp_TechicalSupportSection,
					needHelp_TechicalSupport_img, needHelp_TechicalSupport_phone, needHelp_TechicalSupport_tty,
					needHelp_TechicalSupport_wkDayHrs, null);

			validateSection = "Need Help - Plan Support";
			validateNeedHelpSectionContent(validateSection, needHelp_PlanSupportSection, needHelp_PlanSupport_img,
					needHelp_PlanSupport_phone, needHelp_PlanSupport_tty, needHelp_PlanSupport_wkDayHrs, null);
		}
	...
	public void validateNeedHelpSectionContent(String section, WebElement SectionElement, WebElement imgElement,
			WebElement phoneElement, WebElement ttyElement, WebElement hrsOperationElement1,
			WebElement hrsOperationElement2) {
	...
	/**
	 * Helper method to go back to prior page via browser back, also handles the
	 * case if combo tab is involved note: Tab structure is Suppressed if the
	 * information provided on both the tabs is exactly similar. note: So in this
	 * case we not show the P&P page in case of a standalone ship member note: and
	 * will show the P&P page without any Plan Tabs in case of a combo member
	 * 
	 * @param planType
	 * @param memberType
	 * @param originalUrl
	 * @throws InterruptedException
	 */
	public void goBackToPriorPnPpgViaBack(String planType, String memberType) throws InterruptedException {
		driver.navigate().back();
		CommonUtility.checkPageIsReady(driver);
		String expUrl = "/member/pharmacy/overview.html";
		String actUrl = driver.getCurrentUrl();
		Assert.assertTrue("PROBLEM - unable to go back to PnP page. " + "Expect URL contain '" + expUrl
				+ "' | Actual URL='" + actUrl + "'", actUrl.contains(expUrl));
		// tbd handleComboTabIfComboUser(planType, memberType);
	}

	/**
	 * Navigate to specific plan for combo user, default will fail it if user
	 * doesn't have combo
	 * 
	 * @param planType
	 */
	public void goToSpecificComboTabOnTargetPage(String planType) throws InterruptedException {
		...
		if (planType.equalsIgnoreCase("MAPD")) {
			Assert.assertTrue("PROBLEM - unable to locate combo tab for MAPD", pnpValidate(comboTab_MAPD));
			targetTab = comboTab_MAPD;
		} else if (planType.equalsIgnoreCase("SHIP") || planType.equalsIgnoreCase("MEDSUPP")) {
			Assert.assertTrue("PROBLEM - unable to locate combo tab for SHIP", pnpValidate(comboTab_SHIP));
			targetTab = comboTab_SHIP;
		} else if (planType.equalsIgnoreCase("PDP")) {
			Assert.assertTrue("PROBLEM - unable to locate combo tab for PDP", pnpValidate(comboTab_PDP));
			targetTab = comboTab_PDP;
		} else if (planType.equalsIgnoreCase("SSUP")) {
			Assert.assertTrue("PROBLEM - unable to locate combo tab for PDP", pnpValidate(comboTab_SSUP));
			targetTab = comboTab_SSUP;
		} else {
			Assert.assertTrue(
					"PROBLEM - need to enhance code to cover " + "planType '" + planType + "' for combo testing",
					false);
		}
		targetTab.click();
		targetTab.click();
	...
	/**
	 * Helper method to go to a specific combo tab based on given planType on PnP
	 * page
	 * 
	 * @param planType
	 * @throws InterruptedException
	...
	public String getInfoInConsumerDetails(String planType, String memberType, String infoType) {
		...
		// note: if first / last name, no need to go into planProfiles - infoType:
		// firstName | lastName
		// note: LIS and segmentID needs to get within planProfiles - infoType:
		// segmentId | planCategoryId
		Assert.assertTrue(
				"PROBLEM - code only support locating the following info "
						+ "at the moment: firstName, lastName, segmentId, planCategoryId | Actual='" + infoType + "'",
				infoType.equals("firstName") || infoType.equals("lastName") || infoType.equals("segmentId")
						|| infoType.equals("planCategoryId"));
		...
		try {
			...
			Assert.assertTrue(
					"PROBLEM - unable to locate " + infoType + " from localStorage.consumerDetails, "
							+ "please double check input data planType matches user's actual planType",
					resultInfo != null);
		} catch (JSONException e) {
			e.printStackTrace();
			Assert.assertTrue("PROBLEM - encounted problem reading the json result from localStorage.consumerDetails",
					false);
		}
		return resultInfo;
	}
