...
public class LoginPage extends UhcDriver {
		...
		if ("YES".equalsIgnoreCase(MRScenario.isTestHarness) & "YES".equalsIgnoreCase(MRScenario.isHSIDCompatible)) {
			...
			if ("team-ci1".equalsIgnoreCase(MRScenario.environment)
				...
				PAGE_URL = MRConstants.LEGACY_TESTHARNESS;
					...
					|| (MRScenario.environment.contains("team-voc"))) {
	...
	public Object loginWithLegacy(String username, String password) throws InterruptedException {
		...
		do {
			...
			if (counter < 35) {
				Assert.assertTrue(
						"PROBLEM - got 'You have successfully logged out' message while attempting to land on testharness page",
						!validate(loggedOutMsg, 0));
			...
			counter++;
				...
				|| (driver.getTitle().contains("No Email"))));

		if (currentUrl().contains("login/no-email.html")) {
			driver.get("https://" + MRScenario.environment
					+ "-medicare.ocp-ctc-dmz-nonprod.optum.com/content/medicare/member/testharness.html");
		}
	...
	public Object loginWithLegacy(String username, String password, String userSelection,
			boolean testHarnessUseDropdown) throws InterruptedException {
		// tbd public Object loginWithMicroApp(String username, String password, String
		// userSelection) throws InterruptedException {
		// tbd public Object loginWithMicroApp(String userSelection) throws
		// InterruptedException {
		System.out.println(
				"TEST - username=" + username + " | password=" + password + " | userSelection=" + userSelection);
		...
		/*
		 * keep - re-enable this once the data are mocked on team-atest if
		 * (validate(userSelectionDropDown,0)) { selectFromDropDownByText(driver,
		 * userSelectionDropDown, userSelection); } else { sendkeysNew(userNameField,
		 * username); sendkeysNew(passwordField, password); }
		 */

		/*
		 * tbd if (validate(userNameField,0)) { sendkeysNew(userNameField, username);
		 * sendkeysNew(passwordField, password); } else {
		 * selectFromDropDownByText(driver, userSelectionDropDown, userSelection); }
		 */
		...
		do {
			...
			if (counter < 35) {
				Assert.assertTrue(
						"PROBLEM - got 'You have successfully logged out' message while attempting to land on testharness page",
						!validate(loggedOutMsg, 0));
				if (MRScenario.environment.contains("team-a")) { // note: sometimes take longer to load page on this
																	// team env
					Thread.sleep(5000);
					System.out.println("Time elapsed post sign In clicked --" + counter + "*5 sec.");
				} else {
		...
		if (currentUrl().contains("member/testharness.html")) {
			/*
			 * tbd //vvv note: temp-workaround for team-a env for now if
			 * (MRScenario.environmentMedicare.equalsIgnoreCase("team-a") ||
			 * MRScenario.environmentMedicare.equalsIgnoreCase("team-f")) { return new
			 * AccountHomePage(driver); } //^^^ note: temp-workaround for team-a env for now
			 */
			return new TestHarness(driver);
		} else if (currentUrl().contains("terminated-plan.html")) {
			return new TerminatedHomePage(driver);
