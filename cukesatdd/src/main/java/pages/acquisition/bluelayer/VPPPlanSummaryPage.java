...
public class VPPPlanSummaryPage extends UhcDriver {
	...
	// module-tabs-tabs']/div[2]//a[@class='trigger-closed'][text()='View Plans']")
	...
	// @FindBy(xpath="//*[contains(text() , 'Plan Selector
	// Tool')]/ancestor::div[@class='rightrail']//div[@class='uhc-container']//span")
	...
	/*
	 * tbd private String savePlanLinkTextXpath=
	 * "//span[contains(text(),'Save Plan')]"; private String
	 * savePlanImgXpath="//img[contains(@src,'ic_favorite-unfilled.png')]"; private
	 * String savedPlanLinkTextXpath= "//span[text()='Saved']"; private String
	 * savedPlanImgXpath="//img[contains(@src,'ic_favorite-filled.png')]";
	 */
...
//							jsClickNew(maPlansViewLink);
	...
	public AepVppPlanSummaryPage validate_aepPlanYearLinks(String currentYear, String nextYear) {
		...
		// driver.findElements(By.xpath("//*[contains(text(), '"+nextYear+"')]"));
	 ...
	 * if (element.getText().contains(planName)) { return true; } else { return
	 * false; } }
	 */
	/*
	 * public void validateMedicalBenefitDrugSection() {
	 * validateNew(drugCoveredInfo); validateNew(estimatedAnnualDrigCostLabel);
	...
	public void validateAndClickAddtoCompareinUMS(String planType, String planName) throws InterruptedException {
		if (planType.equalsIgnoreCase("MAPD")) {
			...
			// swiper-slide ng-scope']//input[@id='compare-plan-2']"));
	...
	public void validateAndClickLearnMoreAboutExtraHelpInUMS(String planType, String planName)
			throws InterruptedException {
		if (planType.equalsIgnoreCase("MAPD")) {
			int attempts = 0;
			while (attempts < 2) { // this code is used to prevent staleelement exception
				try {
					WebElement learnMoreAboutExtraHelp = driver.findElement(By.xpath("//*[contains(text(), '" + planName
							+ "')]/ancestor::div[contains(@class, 'module-plan-overview')]//a[contains(@class, 'inline-edit-link modal-link vpp-monthly-premium-modal')]"));
...
//  driver.switchTo().window(parentHandle);
		...
		/*
		 * driver.switchTo().window(parentHandle); if
		 * (driver.getCurrentUrl().contains("plan-summary")) {
		 * System.out.println("Back to VPP Plan Summary Page"); Assert.assertTrue(true);
		 * } else Assert.fail("Unable to navigate back to VPP Plan Summary Page"); }
		 */
		else
...
//vvv note: added for US1598162
	...
	public void validatePrintOptionExistOnPage(String planType) {
		...
		/*
		 * tbd-remove //note: temperary - remove the following when email option is
		 * enable, this is just to make sure it wasn't turn on by mistake WebElement
		 * emailElement=null; if (planType.equalsIgnoreCase("mapd") ||
		 * planType.equalsIgnoreCase("ma")) { //System.out.
		 * println("TEST - going to validate the print and email element for MA");
		 * emailElement=maEmailOption; } else if (planType.equalsIgnoreCase("pdp")) {
		 * //System.out.
		 * println("TEST - going to validate the print and email element for PDP");
		 * emailElement=pdpEmailOption; } else if (planType.equalsIgnoreCase("snp")) {
		 * //System.out.
	...
	public void validateDefaultNoSavedPlan(String planType) {
		...
		List<WebElement> listOfUnsavedPlans = driver.findElements(By.xpath(testXpath
		...
		// xpath="+testXpath+unsavePlanXpath);
		// System.out.println("TEST - Number of unsave plan link for
		// planType='"+planType+"'="+numOfUnsavedPlans);

		List<WebElement> listOfUnfilledIcons = driver.findElements(By.xpath(testXpath
	...
	public VPPPlanSummaryPage navagateToShopAPlanAndFindZipcode(String zipcode, String countyName,
		...
		d.until(ExpectedConditions
				.presenceOfElementLocated(By.xpath("//*[contains(text(),'" + zipcode + " " + countyName + "')]")));
...
//			d.until(ExpectedConditions.elementToBeClickable(By.xpath("//label[contains(@for, 'next_Year')]")));
	...
	/*
	 * --------------------- ArrayList<String> providersForCompare=new
	 * ArrayList<String>();
	 * 
	 * if (providersForCompare > 4) { System.out.
	 * println("There are more than 4 providers, only first 4 will be compared");
	 * providersForCompare=4; } if(ProvidersCovered !=null){ for(int i = 0;
	 * i<providersForCompare; i++){ ProvidersCovered.get(i).click();
	 * System.out.println("Plan added to compare : "+i); } } return
	 * providersForCompare;
	 * 
	 * --------------------------
	 */
