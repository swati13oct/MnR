	...
	public boolean validate_SEPoptions_for_planType(String planType) {
		...
		if (planType.contentEquals("MA")) {
			if (validate(OtherReason) && validate(NoneApply) && validate(LosingCoverage_Employer)
					&& validate(MovedOutside_ServiceArea) && validate(Into_LongTerm) /* && validate(OutOf_LongTerm) */
					&& validate(Disaster) /* && validate(DualSEP) */ /* && validate(ChangeDual) */
					&& validate(ChangeLIS) && validate(
							Assignment) /*
				...
				System.out.println(
						"All Options for MA/MAPD Plan are NOT displayed in SEP page OLE flow : Validation Failed");
				Validation_Flag = false;
			}
		}
		...
		if (planType.contentEquals("PDP")) {
			if (validate(OtherReason) && validate(NoneApply) && validate(LosingCoverage_Employer)
					...
					&& validate(Disaster) /* && validate(DualSEP) && validate(ChangeDual) */ && validate(ChangeLIS)
					&& validate(Assignment)
			/*
			 * && validate(Both_Medicare_Medicaid) && validate(ExtraHelp_PrescriptionDrug)
			 */) {// && validateNonPresenceOfElement(LoSS_LIS)
		...
		if (planType.contentEquals("SNP")) {
			if (validate(OtherReason) && validate(NoneApply) && validate(LosingCoverage_Employer)
					&& validate(MovedOutside_ServiceArea)) {
		...
		for (String currentOption : options) {
			...
			try {
				...
				if (validate(dataTextBx)) {
					...
					sendKeysByCharacter(dataTextBx, currentOptionData); // All date fields are of input tag and sometime
																		// enter incorrect values hence sending keys a
																		// character at a time
				}
			} catch (Exception e) {
			}
