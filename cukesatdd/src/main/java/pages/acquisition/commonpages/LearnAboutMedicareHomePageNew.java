...
public class LearnAboutMedicareHomePageNew extends GlobalWebElements {
	...
	public boolean checkAllLinks() {
		if (validateNew(pageHeader) || validateNew(lnkBenefits) || validateNew(lnkCostBasics)
				|| validateNew(lnkCoverageChoices) || validateNew(lnkEnrollment) || validateNew(lnkLearnMoreMedicareIEP)
				|| validateNew(lnkLearnMoreMedicareWP65) || validateNew(lnkMAPlans)
				|| validateNew(lnkMedicareEligibility) || validateNew(lnkMedSuppPlans)
				|| validateNew(lnkOriginalMedicare) || validateNew(lnkOverviewOfPlans) || validateNew(lnkPDPPlans)
				|| validateNew(lnkSNPPlans)) {
			return true;
		} else {
			return false;
		}
	...
	/*
	 * WebElement
	 * tabRetirement=driver.findElement(By.xpath("//button[contains(@class,'tab2')]"
	 * )); WebElement
	 * tabDisability=driver.findElement(By.xpath("//button[contains(@class,'tab3')]"
	 * ));
	 */
	...
	public void checkEDCTab1() {
		...
		WebElement lblDate = driver
				.findElement(By.xpath("//label[contains(text(),'Enter date of birth (MM/DD/YYYY)')]"));
		Assert.assertTrue(lblDate.getText().contains("Enter date of birth (MM/DD/YYYY)"),
				"EDC Tab 1 Date Field Label Not Verified");
		WebElement txtDate = driver.findElement(By.xpath("//input[@id='tab1-inputfield']"));
		validateNew(txtDate);
		txtDate.sendKeys("11111111");
		WebElement lblError = driver
				.findElement(By.xpath("//input[contains(@id,'tab1')]/following-sibling::p[@class='errorMsg']"));
		...
		List<WebElement> calendars = driver
				.findElements(By.xpath("//div[contains(@class,'content1')]//div[contains(@class,'datecalender')]"));
	...
	public void checkEDCTab2() {
		...
		Assert.assertTrue(tab2Heading.getText().contains(
				"Enter your retirement date or the date you're losing your employer coverage (whichever happens first)"),
				"EDC Tab 1 Header not Verified");
		WebElement lblDate = driver.findElement(By.xpath("//label[contains(@for,'tab2')]"));
		Assert.assertTrue(lblDate.getText().contains("Date you're losing coverage (MM/YYYY)"),
				"EDC Tab 2 Date Field Label Not Verified");
		WebElement txtDate = driver.findElement(By.xpath("//input[@id='tab2-inputfield']"));
		validateNew(txtDate);
		txtDate.sendKeys("111111");
		WebElement lblError = driver
				.findElement(By.xpath("//input[contains(@id,'tab2')]/following-sibling::p[@class='errorMsg']"));
		Assert.assertTrue(lblError.isDisplayed(), "Error Message Not Displayed");
		sleepBySec(2);
		txtDate.clear();
		...
		List<WebElement> calendars = driver.findElements(
				By.xpath("//div[contains(@class,'content2')]//div[contains(@class,'calenderContainer')]"));
		if (calendars.size() == 2 && calendars.get(0).isDisplayed() && calendars.get(1).isDisplayed()) {
			System.out.println("Both Calendars are displayed");
		} else {
			Assert.fail("Both Calendars are not displayed");
		...
		List<WebElement> markedMonths = driver
				.findElements(By.xpath("//div[contains(@class,'content2')]//div[contains(@class,'dob')]"));
		String months = "";
	...
	public void checkEDCTab3() {
		...
		Assert.assertTrue(tab3Heading.getText().contains("Choose a radio button if you have the following:"),
				"EDC Tab 1 Header not Verified");
		...
		WebElement btnGetResult = driver
				.findElement(By.xpath("//button[contains(@onclick,'EDC')]//span[@class='uhc-button__text']"));
		WebElement btnstartOver = driver.findElement(By.xpath(
				"//div[contains(@class,'ESRD')]//following-sibling::div[@class='EDCResetRadio']//a[@class='startAgainRadio']"));
		...
		Assert.assertTrue(lblDate.getText().contains("Enter date you began receiving disability benefits (MM/YYYY)"),
				"EDC Tab 3 Date Field Label Not Verified");
		WebElement txtDate = driver.findElement(By.xpath("//p[@class='radioInputLabel']//following-sibling::input"));
		validateNew(txtDate);
		txtDate.sendKeys("111111");
		WebElement lblError = driver
				.findElement(By.xpath("//p[@class='radioInputLabel']/following-sibling::p[@class='errorMsg']"));
		Assert.assertTrue(lblError.isDisplayed(), "Error Message Not Displayed");
		sleepBySec(2);
		txtDate.clear();
		txtDate.sendKeys("112020");
		...
		if (IEPDate.isDisplayed()
				&& IEPDate.getText().contains("Initial Enrollment Period: Began receiving disability benefits in")) {
		...
		List<WebElement> calendars = driver.findElements(
				By.xpath("//div[contains(@class,'content3')]//div[contains(@class,'calenderContainer')]"));
		if (calendars.size() == 2) {
			System.out.println("Both Calendars are displayed");
		} else {
			Assert.fail("Both Calendars are not displayed");
		...
		List<WebElement> markedMonths = driver
				.findElements(By.xpath("//div[contains(@class,'content3')]//div[contains(@class,'dob')]"));
		String months = "";
	...
	public void validateWizardComponent() {
		...
		String ques2 = "Is your health insurance through your spouse" + String.valueOf(Character.toChars(8217))
				+ "s employer?";
		...
		String answer = "Because you" + String.valueOf(Character.toChars(8217))
				+ "re receiving Social Security / Railroad Retirement Board benefits, you will be automatically enrolled in Medicare Parts A & B when you turn 65. However, since your employer has 20 or more employees, you may be able to delay enrollment if you have creditable drug coverage!";
	...
	public void validatePDFViewer() {
		...
		WebElement btnSearchCancel = driver.findElement(
				By.xpath("//div[contains(@class,'SearchInputUI')]//button//span[contains(text(),'Cancel')]"));
	...
	public void validatePdfMenuDownloadLink(String pdfName) {
		...
		WebElement btnDownload = driver
				.findElement(By.xpath("//button[contains(@class,'Button')]//span[contains(text(),'Download PDF')]"));
		if (MRScenario.browserName.equalsIgnoreCase("chrome")) {
			...
			WebElement pdfDownload = (WebElement) executor.executeScript(
					"return document.querySelector('downloads-manager').shadowRoot.querySelector('#mainContainer').querySelector('downloads-item').shadowRoot.querySelector('#content.is-active').querySelector('#details')");
	...
	public void validatePdfMenuPrintLink() {
		if (MRScenario.browserName.equalsIgnoreCase("chrome")) {
			...
			WebElement btnPrint = driver
					.findElement(By.xpath("//button[contains(@class,'Button')]//span[contains(text(),'Print PDF')]"));
			jsClickNew(btnPrint);
			sleepBySec(2);
