...
public ComparePlansPage providerfromMedicalGroup() throws Exception {
		...
		/*
		 * Set<String> handles1 = driver.getWindowHandles(); for (String windowHandle :
		 * handles1) { if (!windowHandle.equals(ParentWindow)) {
		 * driver.switchTo().window(windowHandle); String title = driver.getTitle();
		 * System.out.println("Window title is : " + title); if
		 * (title.contains("Compare Medicare Plans")) {
		 * System.out.println("We are on Compare Medicare Plans winodow opened");
		 * driver.manage().window().maximize(); Thread.sleep(3000);
		 * waitforElement(editDoctorsLink); break; } } else {
		 * System.out.println("Not found Expected window");
		 * driver.switchTo().window(ParentWindow); }
		 * 
		 * }
