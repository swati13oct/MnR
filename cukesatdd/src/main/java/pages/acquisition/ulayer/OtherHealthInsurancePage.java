	...
	public void entersotherhealthinsurInformation(
		...
		if(othradiooption.equalsIgnoreCase("No")){
			...
			public boolean validateOTHPage(){
				boolean flag = false;
				if(validate(otherhealthinsuranceno)&&validate(otherhealthinsuranceyes)&&validate(otherhelathinsurancesaveandcontinue)&&validate(otherhealthinsuranceprevious)&&
