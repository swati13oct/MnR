...
public class VPPPlanSummaryPage extends UhcDriver {
	...
	public PlanDetailsPage navigateToPlanDetails(String planName, String planType) {
		...
		if (planType.equalsIgnoreCase("MA") || planType.equalsIgnoreCase("MAPD")) {
			...
			ElementData elementData = new ElementData("id", "viewDetails" + planType);
	...
	private WebElement getViewPlanDetailsElement(List<WebElement> planElement, ElementData elementData,
			String planName) {
	...
	public PlanInformationPage navigatetoEnrollInplanLink(String planName) {
		...
		if (driver.getTitle().equalsIgnoreCase("AARP Medicare Complete Online Application")
				|| driver.getTitle().equalsIgnoreCase("AARP Medicarerx Online Application")) {
	...
	public ProviderSearchPage clicksOnIsProviderCovered(String planName) {
		...
		return null;
		/*
		 * for (WebElement physcianSearchType : physcianSearchTypes) { if
		 * (physcianSearchType.getText().contains(physicianSearchCriteria)) {
		 * ElementData physcianElementdata = new ElementData("tagName", "a");
		 * findChildElement(physcianElementdata,physcianSearchType).click(); } }
		 * 
		 * 
		 * for(WebElement element : providerNameList){ ElementData
		 * providerElementData = new ElementData("className", "providerName");
		 * if(findChildElement(providerElementData,element).getText().
		 * equalsIgnoreCase(physicianName)) { ElementData addToListElementData =
		 * new ElementData("linkText", "Add to List");
