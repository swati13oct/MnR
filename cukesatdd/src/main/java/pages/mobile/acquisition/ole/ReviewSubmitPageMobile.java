...
public class ReviewSubmitPageMobile extends UhcDriver {
	...
	// @FindBy(xpath = "//*[contains(text(), 'Medicare Claim Number') or
	// contains(text(), 'Medicare (Claim) Number')]//following-sibling::*")
	...
	// @FindBy(xpath = "//*[contains(text(), 'mailing
	// address')]/ancestor::*[contains(@class, 'review-step')]//*[contains(text(),
	// 'Street Address')]//following-sibling::*")
	private List<WebElement> StreetDisplays;

	@FindBy(xpath = "//*[contains(text(), 'City')]//following-sibling::*")
	// @FindBy(xpath = "//*[contains(text(), 'mailing
	// address')]/ancestor::*[contains(@class, 'review-step')]//*[contains(text(),
	// 'City')]//following-sibling::*")
	...
	// @FindBy(xpath = "//*[contains(text(), 'Member
	// Number')]//following-sibling::*")
	...
	public boolean OnlineEnrollment_Review_Page_details(Map<String, String> detailsMap) {
		...
		// 'mailing address')]/ancestor::*[contains(@class,
		// 'review-step')]//*[contains(text(), 'State')]//following-sibling::*"));
		...
		if (Expected_PlanName.contains("Chronic") || Expected_PlanName.contains("Gold")
				|| Expected_PlanName.contains("Silver")) {
		 ...
		 * if(validate(Submit_Disclaimer) && validate(Enrollment_Disclaimer_Text)){
		 * if(Enrollment_Disclaimer_Text.getText().
		 * contains("Submitting your enrollment application electronically")){ //flag =
		 * (!flag)?false:true;
		 * System.out.println("Submit Enrollment Disclaimer is Displayed  : "+flag); }
		 * else flag = false; }else flag = false;
		 * 
		 * if(validate(SubmitApplicationBtn)){ if(SubmitApplicationBtn.isEnabled()){
		 * //flag = (!flag)?false:true;
		 * System.out.println("Submit Application Button is displayed and Enabled : "
		 * +flag); } else flag = false; }else flag = false;
		 */
		if (flag) {
	...
	public boolean validateTextPlanName(WebElement element, String expectedValue) {
		...
		if (!StringUtils.isEmpty(expectedValue)) {
			...
			if (!result) {
				System.out.println("Review and Submit Pages validation failed for PlanName-----------------------" + " "
						+ element.getText());
			}
		}
		return result;
	...
	public boolean validateText(WebElement element, String expectedValue) {
		...
		if (!StringUtils.isEmpty(expectedValue)) {
			...
			System.out.println(expectedValue + " " + element.getText() + " " + result);
			if (!result) {
				System.out.println("Review and Submit Pages validation failed for -----------------------" + " "
						+ element.getText());
			}
		}
		return result;
	...
	public boolean Review_page_enter_required_Medicare_details(Map<String, String> MedicareDetailsMap)
		...
		scrollToView(EditMedicareInformation);
		jsClickNew(EditMedicareInformation);

		validate(claimNumberField);
		sendKeysByCharacter(claimNumberField, MedicareNumber1);
		Thread.sleep(2000);
		sendKeysByCharacter(medicaidNumberField, "12345876");
		Thread.sleep(2000);
		 ...
		 * System.out.println("User navigate back to Review Page"); return true; } else
		 * System.out.
		 * println("savechanges is disabled, User not navigated back to Review Page");
		 * return false;
		 */
		return result;
	}
