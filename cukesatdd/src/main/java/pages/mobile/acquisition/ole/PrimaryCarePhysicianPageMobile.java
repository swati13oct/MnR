 ...
 *
 */
public class PrimaryCarePhysicianPageMobile extends UhcDriver {
...
//	@FindBy(css = "#hasCurrentPatientOfPcpNo")
	...
	public boolean validate_provider_contact_info_in_PCP() {
		...
		if (validate(ProviderINfoHeader) && validate(Doctor1_Name) && validate(Doctor1_Ph) && validate(Doctor1_City)
				&& validate(Doctor1_State) && validate(Doctor1_Zip) && validate(Doctor2_Name) && validate(Doctor2_Ph)
				&& validate(Doctor2_City) && validate(Doctor2_State) && validate(Doctor2_Zip) && validate(Hospital_Name)
				&& validate(Hospital_Ph) && validate(Hospital_City) && validate(Hospital_State)
				&& validate(Hospital_Zip)) {
	...
	public boolean validate_provider_Lookup(String planType) {
		...
		if (validate(LookUpProviderBtn)) {
			...
			if (CurrentURL.contains("post-error")) {
				System.out.println(
						"Provider look Up Button CLicked - Rally Provider Look up Error Page is displayed for Plan Type : "
								+ planType);
...
//					}
		...
		/*
		 * else{ System.out.println("Provider Look Up button is not displaye");
		 * validation_Flag = false; }
		 */
	...
	public PlanPremiumPageMobile navigate_to_Plan_Premium_Page() {
		 ...
		 */
		// if(validateNew(driver.findElement(By.xpath("//h1[contains(text(),'Plan
		// Premium')]")))){
	...
	public PrimaryCarePhysicianPageMobile navigate_PCPPage() {
		...
		// WebElement radioBtn =
		// driver.findElement(By.xpath("//*[contains(@class,'ole-provider-list')]//ul[@class='ul-pcp-list']//li[@class='active']"));
		WebElement radioBtn = driver.findElement(By.xpath(
				"	//*[contains(@class,'ole-provider-list')]//ul[@class='ul-pcp-list']//li[contains(@class,'ng-star-inserted active')]"));
...
//		String expectedProvider= driver.findElement(By.xpath("//p[text()='Doctor/Provider or PCP Full Name: ']//following-sibling::p[contains(@class,'provider-info__data')][1]")).getText().trim();
		String expectedProvider = driver.findElement(By.xpath(
				"//p[contains(text(),'Provider or PCP Full Name: ')]//following-sibling::p[contains(@class,'provider-info__data')][1]"))
				.getText().trim();
//		String PCPNumber= driver.findElement(By.xpath("//p[text()='Doctor/Provider/PCP Number: ']//following-sibling::p[contains(@class,'provider-info__data')]")).getText();
		String PCPNumber = driver.findElement(By.xpath(
				"//p[contains(text(),'Provider/PCP Number: ')]//following-sibling::p[contains(@class,'provider-info__data')]"))
				.getText();
	...
	public ArrayList<String> pcpinforetreive(String plantype) {
		...
		// WebElement PCPSearchLink =
		// driver.findElement(By.xpath("//button[@class='view-more-btn-pcp']"));
