...
public class DrugCostEstimatorPageMobile extends UhcDriver {
	...
	public void validateEditDrugAndPharmacyLinks() {
		Assertion.assertTrue(elementFound(step3EditDrugsList));
		Assertion.assertTrue(elementFound(step3EditPharmacyList));
	}
...
//		}
	...
	public void validatePlanYear(){
		if(year2017.isDisplayed() && year2018.isDisplayed()){
			Assertion.assertTrue("Plan year are displayed", true);
			year2017.click();
			year2018.click();
		}else{
			Assertion.assertFalse("Plan year are not displayed", true);
		}
	}
	...
	public void checkProactiveChatPopup(){
		CommonUtility.waitForPageLoad(driver, proactiveChatExitBtn,20); // do not change this to CommonUtility.waitForPageLoadNew as we're not trying to fail the test if it isn't found
		try{
	...
	public void validateLocalStorage(Map<String, String> dCEAttributesMap) {
		...
		Assertion.assertTrue("UHC Local Storage Validation Failed",validation_Flag);		
	}
	
	public DrugCostEstimatorPageMobile validateDCEPageDisplayed() {
		waitforElementNew(drugCostEstimatorPageHeaderText,15);
		Assertion.assertTrue("DCE page is not loaded", drugCostEstimatorPageHeaderText.getText().equals("Drug Cost Estimator"));
		return null;
	}
