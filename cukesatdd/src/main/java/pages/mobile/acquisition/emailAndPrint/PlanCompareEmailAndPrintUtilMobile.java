...
public class PlanCompareEmailAndPrintUtilMobile extends EmailAndPrintUtilBaseMobile {
	...
	public HashMap<String, String> collectInfoVppPlanComparePg(String planType, String forWhat, WebDriver origDriver) {
		...
		for (int i = 1; i <= listOfCmpPlansColumns.size(); i++) {
			...
			String enrollXpath = columnXpath
					+ "//div[@id='innerdiv' and not(contains(@class,'ng-hide'))]//a[contains(@id,'enrollbtnplancompare')]";
			...
			if (tmp.size() == 1) {
				...
				for (int j = 1; j <= cellsPerRow; j++) {
					...
					try {
						...
						System.out.println(
								"unable to find this element with xpath='" + cellXpath + "', ignore it, move on");
					}
				}
			}
		}
	...
	public void validatePlanCompareEmailThankYouMessage() {
		...
		// Click issue on plan compare page for Email validation , temporary skipping it as no solution yet found 
		
		/*
		 * jsClickNew(validateemailbutton); //pageloadcomplete(); // Validating email
		 * popup validate(leavingcomapreplansitepopup);
		 * System.out.println("!!!Leaving site popup is displayed ===>"); // Validating
		 * email cancel button validate(cancelButtonEmailPlanComparePopUp);
		 * System.out.println("!!!Cancel Button is displayed ===>");
		 * cancelButtonEmailPlanComparePopUp.click();
		 * System.out.println("Success click of cancel email"); // Validating email send
		 * button // validateemailbutton.click(); jsClickNew(validateemailbutton);
		 ...
		 * "rani_madadi@optum.com"); System.out.println("!!!Entered valid Email "); //
		 * sendButtonEmailPlanComparePopUp.click();
		 * jsClickNew(sendButtonEmailPlanComparePopUp); Assertion.
		 * assertTrue("PROBLEM - unable to get success message after clicking send",
		 * validate(validatesuccesspopup)); // Validating email success popup
	...
	public String compare_comparePageItem(String targetKey, HashMap<String, String> origPage,
			HashMap<String, String> emailage) {
	...
	public List<String> validatePlanCompareEmailDeeplink(String planType, String deepLinkStringId,
			String infoMapStringId, String deepLink, HashMap<String, String> origPage, WebDriver origDriver) {
		String forWhat = "from deepLink";
		driver.manage().timeouts().implicitlyWait(0, TimeUnit.SECONDS);
		List<String> testNote = new ArrayList<String>();
		System.out.println(
				"Proceed to validate the original page content vs page content from email deeplnk for plan compare...");
		...
		System.out.println(
				"Finished validation for the original page content vs page content from email deeplnk for plan compare ===========");
