...
public class EnterZipCodePageMobile extends UhcDriver {
	...
	/*
	 * public void validateZipComp(String zipCode) { try { int zipCodeNumber = 1;
	 * System.out.println("Total " + zipForm.size() +
	 * " Zip code component[s] display on page");
	 * 
	 * while (zipCodeNumber <= zipForm.size()) { Thread.sleep(3000);
	 * ZipCodeText.get(zipCodeNumber - 1).clear(); ZipCodeText.get(zipCodeNumber -
	 * 1).sendKeys(zipCode); ZipcodeButton.get(zipCodeNumber - 1).click();
	 ...
	 * Thread.sleep(20000); String vppPageTitle = driver.getTitle(); if
	 * (driver.getWindowHandles().size() > 1) { String currentPage =
	 * driver.getWindowHandle(); Set<String> newWindow = driver.getWindowHandles();
	 * for (String tabs : newWindow) { if (!tabs.equalsIgnoreCase(currentPage))
	 * vppPageTitle = driver.switchTo().window(tabs).getTitle(); } }
	 * 
	 * System.out.println("Actual : " + vppPageTitle);
	 * if(driver.getCurrentUrl().contains("aarpmedicareplans")) {
	 * System.out.println("Expected : " +
	 * PageTitleConstants.ULAYER_VPP_PLAN_PAGE_AARP_MEDICARE);
	 ...
	 * PageTitleConstants.BLAYER_VPP_PLAN_PAGE_AARP_MEDICARE);
	 * assertTrue("Not redirected to VPP page",
	 * vppPageTitle.contains(PageTitleConstants.BLAYER_VPP_PLAN_PAGE_AARP_MEDICARE))
	 * ; } driver.navigate().back(); zipCodeNumber++; } } catch (Exception e) {
				...
				if (driver.getCurrentUrl().contains("aarpmedicareplans")) {
					if (vppPageTitle.replaceAll("[^A-Za-z0-9:.]", "").contains(
						...
						System.out.println("Page Title : " + PageTitleConstants.ULAYER_PRESCRIPTION_DRUG_SNPPLANS);
							...
							.contains((PageTitleConstants.ULAYER_SHOP_ALL_PLANS).replaceAll("[^A-Za-z0-9:.]", "")))
