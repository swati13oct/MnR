...
public class VPPPlanSummaryPageMobile extends GlobalWebElements {
	...
	// @FindBy(xpath = "//div[contains(@class,'module-tabs-tabs')]/div[not
	// (contains(@class,'active'))]//span[@id='maviewplans']/following-sibling::a")
	@FindBy(css = "a[dtmname$='MA:View Plans']")
	private WebElement maPlansViewLink;

	// @FindBy(xpath = "//div[@class='overview-tabs
	// module-tabs-tabs']/div[4]//a[contains(@class,'trigger-closed')]")
	...
	// @FindBy(xpath = "//*[@class='trigger-closed' and @dtmname='Plans Landing:Plan
	// Box:MS:View Plans']")
	...
	// @FindBy(xpath = "//div[contains(@class,'module-tabs-tabs')]/div[not
	// (contains(@class,'active'))]//span[@id='pdpviewplans']/following-sibling::a")
	...
	public void validateAbilityToSavePlans(String savePlanNames, String planType) {
		...
		for (String plan : listOfTestPlans) {
			...
			scrollToView(planCard);
			Assertion.assertEquals("PROBLEM - " + noOfPlanCards.size() + " plan cards found for " + plan,
					noOfPlanCards.size(), ONE);
			...
			Assertion.assertEquals("PROBLEM - " + noOfSavePlanIcons.size() + " save icons found for " + plan,
					noOfSavePlanIcons.size(), ONE);
			 ...
			 * (!StringUtils.isNullOrEmpty(State)) { if
			 * (State.equalsIgnoreCase("Pennsylvania") ||
			 * State.equalsIgnoreCase("Puerto Rico") || State.equalsIgnoreCase("Virginia"))
			 * { if (validate(closeProfilePopup)) scrollToView(closeProfilePopup);
			 * jsClickNew(closeProfilePopup); } else { if (validate(keepShoppingBtn))
			 * scrollToView(keepShoppingBtn); jsClickNew(keepShoppingBtn); } } else { if
			 * (validate(keepShoppingBtn)) scrollToView(keepShoppingBtn);
			...
			if (closeProfilePopup.isDisplayed()) {
				...
				if (keepShoppingBtn.isDisplayed()) {
					jsClickNew(keepShoppingBtn);
				}
			}
			CommonUtility.checkPageIsReady(driver);
			...
			Assertion.assertEquals("PROBLEM - " + noOfSavedText.size() + " 'Saved' text found for " + plan,
					noOfSavedText.size(), ONE);
			Assertion.assertStringContains("PROBLEM - 'Saved' text is not displayed for " + plan,
					noOfSavedText.get(0).getText(), "Saved");
		}
	...
	public void validatePlansAreSaved(String savePlanNames, String planType) {
		...
		List<String> listOfTestPlans = Arrays.asList(savePlanNames.split(","));
		System.out.println("Validating total number of test plans saved are " + listOfTestPlans.size()
				+ " for plan type " + planType);
		...
		for (String plan : listOfTestPlans) {
			...
			Assertion.assertEquals("Number of saved plans for " + planType + " do not match.", savedPlans.size(),
					listOfTestPlans.size());
		...
		/*
		 * sleepBySec(10); String planTypePath = "";
		 * 
		 * sleepBySec(3); if (planType.equalsIgnoreCase("MA") ||
		 * planType.equalsIgnoreCase("MAPD")) { planTypePath =
		 * "//div[@ng-show='showMaPlans']"; } else if (planType.equalsIgnoreCase("PDP"))
		 * { planTypePath = "//div[@ng-show='showPdpPlans']"; //
		 * driver.navigate().refresh(); CommonUtility.checkPageIsReady(driver);
		 * sleepBySec(5); } else if (planType.equalsIgnoreCase("SNP")) { planTypePath =
		 * "//div[@ng-show='showSnpPlans']"; } List<String> listOfTestPlans =
		 * Arrays.asList(savePlanNames.split(","));
		 * 
		 * System.out.println("Validate " + listOfTestPlans.size() +
		 * " number of test plans are saved as favorite");
		 * 
		 * // String appeared_savedPlanLIconXpath = planTypePath +
		 * "//*[contains(@class, 'added')]" + savedPlanImgXpath; String
		 * appeared_savedPlanLIconXpath = planTypePath +
		 * "//*[contains(@class, 'added') and not(contains(@style,'display:none'))]" +
		 * savedPlanImgXpath;
		 * 
		 * System.out.println("TEST - appeared_savedPlanLIconXpath xpath=" +
		 * appeared_savedPlanLIconXpath); List<WebElement> listOfAppearedSavedPlanIcons
		 * = driver.findElements(By.xpath(appeared_savedPlanLIconXpath)); int expMatch =
		 * listOfTestPlans.size(); CommonUtility.checkPageIsReadyNew(driver);
		 * Assertion.assertTrue(
		 * "PROBLEM - total saved plan icons not as expected.  Expect number of match='"
		 * + expMatch + "' | Actual number of match='" +
	...
	public void validateAbilityToUnSavePlans(String savedPlans, String planType) {
		...
		// System.out.println("Validating total number of test plans saved are " +
		// listOfTestPlans.size() + " for plan type " + planType);
		...
		Assertion.assertEquals("PROBLEM - 'Saved' icon found for plan " + plan + " after clicking unsave",
				savedPlanIcons.size(), ZERO);
		...
		/*
		 * String subPath = determineSubpath(planType); String headerPath =
		 * determineHeaderPath(planType);
		 * 
		 * List<String> listOfTestPlans = Arrays.asList(savedPlans.split(",")); String
		 * unsavePlan = listOfTestPlans.get(0);
		 * System.out.println("Proceed to unsave 1st plan from input '" + unsavePlan +
		 * "'");
		 * 
		 * String testPlanXpath = "//*[contains(text(),'" + unsavePlan +
		 * "') and contains(@class,'ng-binding')]"; WebElement testPlanXpath1 = driver
		 * .findElement(By.xpath("//*[contains(text(),'" + unsavePlan +
		 * "') and contains(@class,'ng-binding')]"));
		 * testPlanXpath1.getText().replaceAll("\u00A00", " ").trim(); List<WebElement>
		 * listOfPlans = driver.findElements(By.xpath(testPlanXpath)); int expMatch = 1;
		 * Assertion.assertTrue("PROBLEM - unable to locate plan='" + unsavePlan +
		 * "'.  Expect number of match='" + expMatch + "' | Actual number of match='" +
		 * listOfPlans.size() + "'", listOfPlans.size() == expMatch);
		 * 
		 * System.out.
		 * println("Proceed to validate 'Saved Plan' icon is there before clicking to unsave it"
		 ...
		 * + savedPlanImgXpath;
		 * 
		 * if (planType.equals("PDP")) { appeared_savedPlanLIconXpath =
		 * "//*[contains(text(),'" + unsavePlan +
		 * "')]/ancestor::*[contains(@class,'module-plan-overview')]//*[contains(@class,'added') and contains(@dlassetid,'lnk-1')]"
		 * + savedPlanImgXpath; }
		 * System.out.println("TEST - appeared_savedPlanLIconXpath xpath=" +
		 * appeared_savedPlanLIconXpath); List<WebElement> listOfAppearedSavedPlanIcons
		 * = driver.findElements(By.xpath(appeared_savedPlanLIconXpath)); expMatch = 1;
		 * Assertion.
		 * assertTrue("PROBLEM - unable to locate Saved Plan icon after click for ='" +
		 * unsavePlan + "'.  Expect number of match='" + expMatch +
		 * "' | Actual number of match='" + listOfAppearedSavedPlanIcons.size() + "'",
		 * listOfAppearedSavedPlanIcons.size() == expMatch);
		 * 
		 * // System.out.println("TEST - appeared_savedPlanLIconXpath xpath=" +
		 * appeared_savedPlanLIconXpath); // List<WebElement>
		 * listOfAppearedSavedPlanIcons =
		 * driver.findElements(By.xpath(appeared_savedPlanLIconXpath)); // expMatch = 1;
		 * // Assertion.
		 * assertTrue("PROBLEM - unable to locate Saved Plan icon after click for ='" +
		 * unsavePlan // + "'.  Expect number of match='" + expMatch +
		 ...
		 * jsClickNew(listOfAppearedSavedPlanIcons.get(0)); try { Thread.sleep(2000); }
		 * catch (InterruptedException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } System.out.
		 * println("Proceed to validate 'Saved Plan' icon is not there after clicking to unsave it"
		 * ); System.out.println("TEST - appeared_savedPlanLIconXpath xpath=" +
		 * appeared_savedPlanLIconXpath); listOfAppearedSavedPlanIcons =
		 * driver.findElements(By.xpath(appeared_savedPlanLIconXpath)); expMatch = 0;
		 * 
		 * Assertion.
		 * assertTrue("PROBLEM - 'Saved Plan' icon should no longer appear for ='" +
		 * unsavePlan + "'.  Expect number of match='" + expMatch +
		 * "' | Actual number of match='" + listOfAppearedSavedPlanIcons.size() + "'",
		 * listOfAppearedSavedPlanIcons.size() == expMatch);
		 * 
		 * System.out.
		 * println("Proceed to validate 'Save Plan' icon appeared again after unsaved plan"
		 ...
		 * + savePlanImgXpath; System.out.println("TEST - savePlanIconXpath xpath=" +
		 * savePlanIconXpath); List<WebElement> listOfSavePlanIcons =
		 * driver.findElements(By.xpath(savePlanIconXpath)); expMatch = 1;
		 * Assertion.assertTrue( "PROBLEM - unable to locate Save Plan icon for ='" +
		 * unsavePlan + "'.  Expect number of match='" + expMatch +
		 * "' | Actual number of match='" + listOfSavePlanIcons.size() + "'",
		 * listOfSavePlanIcons.size() == expMatch);
		 */
	}
	...
	public void validateOnePlanSavedOnePlanUnsaved(String savePlanNames, String planType) {
		...
		for (int planNumber = 0; planNumber < listOfTestPlans.size(); planNumber++) {
			...
			/*
			 * if (i == 1) { // In the previous steps the plan 0 is unsaved so we will
			 * validate plan 1 is // still saved String plan = listOfTestPlans.get(i);
			 ...
			 * List<WebElement> listOfPlans = driver.findElements(By.xpath(testPlanXpath));
			 * System.out.println("TEST - size=" + listOfPlans.size()); int expMatch = 1;
			 * Assertion.assertTrue( "PROBLEM - unable to locate plan='" + plan +
			 * "'.  Expect number of match='" + expMatch + "' | Actual number of match='" +
			 * listOfPlans.size() + "'", listOfPlans.size() == expMatch);
			 * 
			 ...
			 * "')]/ancestor::*[contains(@class,'module-plan-overview')]//*[contains(@class,'added') and not(contains(@style,'display:none'))]"
			 * + savedPlanImgXpath; if (planType.equals("PDP")) { savedPlanIconXpath =
			 * "//*[contains(text(),'" + plan +
			 * "')]/ancestor::*[contains(@class,'module-plan-overview')]//*[contains(@class,'added')and contains(@dlassetid,'lnk-1')]"
			 * + savedPlanImgXpath;
			 * 
			 * } System.out.println("TEST - initial_savePlanLIconXpath xpath=" +
			 * savedPlanIconXpath); List<WebElement> listOfSavePlanIcons =
			 * driver.findElements(By.xpath(savedPlanIconXpath)); expMatch = 1;
			 * Assertion.assertTrue( "PROBLEM - unable to locate Save Plan icon for ='" +
			 * plan + "'.  Expect number of match='" + expMatch +
			 * "' | Actual number of match='" + listOfSavePlanIcons.size() + "'",
			 * listOfSavePlanIcons.size() == expMatch); } else if (i == 0) { // In the
			 * previous steps the plan 0 is unsaved so we will validate plan 0 is // still
			 * unsaved String plan = listOfTestPlans.get(i); System.out.println("Plan '" +
			 * plan + "' should be unsaved");
			 * 
			 * String testPlanXpath = "//*[contains(text(),'" + plan +
			 * "') and contains(@class,'ng-binding')]"; List<WebElement> listOfPlans =
			 * driver.findElements(By.xpath(testPlanXpath)); int expMatch = 1;
			 * Assertion.assertTrue( "PROBLEM - unable to locate plan='" + plan +
			 * "'.  Expect number of match='" + expMatch + "' | Actual number of match='" +
			 * listOfPlans.size() + "'", listOfPlans.size() == expMatch);
			 * 
			 ...
			 * "')]/ancestor::*[contains(@class,'module-plan-overview')]//*[contains(@aria-selected,'false')]"
			 * + savePlanImgXpath; System.out.println("TEST - savedPlanIconXpath xpath=" +
			 * savePlanIconXpath); List<WebElement> listOfSavedPlanIcons =
			 * driver.findElements(By.xpath(savePlanIconXpath)); expMatch = 1;
			 * Assertion.assertTrue( "PROBLEM - unable to locate Save Plan icon for ='" +
			 * plan + "'.  Expect number of match='" + expMatch +
			 * "' | Actual number of match='" + listOfSavedPlanIcons.size() + "'",
	...
	public void handlePlanYearSelectionPopup(String planYear) {
		 ...
		 * year if (validate(IsMyDoctorCovereredBanner, 20)) {
		 * System.out.println("***** Doctor banner verified ******");
		 * 
		 * 
		 * /* if (planYear.equalsIgnoreCase("current")) { // if the scenario is for
		 * current year if (validate(IsMyDoctorCovereredBanner, 20)) {
		 * System.out.println("***** Doctor banner verified ******");
		 * 
		 * } else { validate(HowMuchDrugCostBanner, 10);
	...
	public GetStartedPageMobile navigateToDCEFromNBA(String planType, String planName) {
