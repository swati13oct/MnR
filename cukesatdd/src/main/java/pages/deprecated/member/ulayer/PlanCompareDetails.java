...
public class PlanCompareDetails extends UhcDriver {
	...
	public boolean isPreferredNetwork() {
		boolean valid;
		valid = validateNew(preferredText) && preferredText.getText().equals("UnitedHealthcare members can save more money on thousands of prescription drugs through our Preferred Retail Pharmacy Network. We've worked with many retail pharmacies in your area to help you save money on your prescription co-pays.");
		if (!valid) return false;
		else if (validateNew(preferredTableText) && preferredTableText.getText().equals("Some plans offer savings for buying a three-month supply via mail rather than buying a one-month supply from a standard retail pharmacy. Prices here are for a three-month supply using the Preferred Mail Service Pharmacy.")) {
			return true;
		}
	...
	public boolean isPreferredNetworkMAPD() {
		boolean valid;
		valid = validateNew(preferredNetworkDisclaimerMAPD) && preferredNetworkDisclaimerMAPD.isDisplayed();
	...
	public boolean isPharmacySaver() {
		boolean valid;
		valid = validateNew(pharmacySaverFooter) && pharmacySaverFooter.isDisplayed();
		if (!valid) return false;
		else if (validateNew(pharmacySaverRightRail) && pharmacySaverRightRail.isDisplayed()) {
			return true;
		}
