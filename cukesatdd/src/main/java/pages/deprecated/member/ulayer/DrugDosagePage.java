...
public class DrugDosagePage extends UhcDriver {
	...
	public Object selectDosage(Map<String, String> dosageAttributesMap) {
		...
		sendkeysNew(quantityField, drugQuantity);
		select(frequencyField, drugFrequency);

		continueButton.click();
		CommonUtility.waitForPageLoadNew(driver, lowCostEditBtn,CommonConstants.TIMEOUT_30);
    ...
    public boolean validateDrugDosagePage(){
    	if(selectDosage.getText().contains("Select dosage")&&validateNew(continueButton)&&validateNew(addADiffDrugLink))
    		return true;
    	else
    		return false;
    }
