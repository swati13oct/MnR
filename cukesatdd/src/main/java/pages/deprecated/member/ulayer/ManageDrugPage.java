...
public class ManageDrugPage extends UhcDriver {
	...
	public AddDrugPage searchDrug(String drugInitials) {
		addDrugLink.click();
		sendkeysNew(drugName, drugInitials);
		try {
			Thread.sleep(5000);
	...
	public SelectPharmacyPage navigateToPharmacyPage() {
		...
		if (validateNew(standardRadioBtn)) 
	...
	public boolean validateManageDrugPage(){
		...
		if(validateNew(addDrugLink)&&validateNew(selectPharmacyBtn))
			return true;
		return false;
	}
	public boolean validateDrugListSection(){
		...
		if(validateNew(addDrugLink)&&validateNew(viewDrugCostTab)&&validateNew(pharmacyTab)&&validateNew(drugListTab)&&validateNew(plans))
			flag = true;
		return flag;
	}
	
	public boolean validateDrugAdded(){
		if(validateNew(drugListBox)&&validateNew(editDrugLink)&&validateNew(deleteLink))
			return true;
		else
			return false;
	}
