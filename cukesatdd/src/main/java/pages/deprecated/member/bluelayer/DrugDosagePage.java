...
public class DrugDosagePage extends UhcDriver {
	...
	public void selectDrugAndValidate(){
		waitforElementNew(clickContinue);
		clickContinue.click();
	}
	...
	public SelectPharmacyPage navigateToWaysToSave(){
		waitforElementNew(selectPharmacy);
		selectPharmacy.click();
	...
	public boolean validateDrugDosageSection(){
		boolean flag = false;
		if(validateNew(addADrugLink)&&validateNew(cancelAndGoBackLink)&&validateNew(quantityField)&&validateNew(frequencyBox)&&validateNew(continueButton))
			flag = true;
		return flag;
	}
}
