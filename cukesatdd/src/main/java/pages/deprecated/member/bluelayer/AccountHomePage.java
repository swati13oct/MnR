...
import pages.deprecated.member.bluelayer.*;
...
driver.switchTo().window(mainwindow);
	...
	{
		CommonUtility.checkPageIsReadyNew(driver);
    CommonUtility.waitForPageLoadNew(driver, medicalProviders, 10000L);
...
System.out.println(" in navigate Provider Search under my Resource");
	...
	public void panelNavigation()
	{
	   ...
	   waitforElementNew(myMenu);
	   myMenu.click();
	
	   waitforElementNew(providerSearchinPanelNavigation);
	...
	public void providerSearchLinkinClaimsPage()
	{
		...
		waitforElementNew(claimsLink);
		claimsLink.click();
		waitforElementNew(searchLinkinClaimsPage);
		
		searchLinkinClaimsPage.click();
		
		waitforElementNew(providerLink);
	...
	public void BenefitsandCoverageProviderSearch()
	{
		...
		waitforElementNew(myPlansTab);
		myPlansTab.click();
		
        waitforElementNew(benefitsAndCoveragelink);
		benefitsAndCoveragelink.click();
		waitforElementNew(medicalEOBproviderlink);
	...
	public void providerSearchRHandWidgetBlayer()
	{
		...
		waitforElementNew(medicalEOBlink);
	...
	public void providerSearchLinkinPCPSection()
	{
		...
		waitforElementNew(providerlinkinPCPSection);
...
public void FormsandResourcesLinkinPlanSummaryPageBlayer()
	
	{
	  ...
	  myPlansTab.click();
	  waitforElementNew(FormsandResourcesLinkn);
	
		FormsandResourcesLinkn.click();
		
		waitforElementNew(searchProviderLinkinFormsandResourcePage);
	...
	public PharmacySearchPage navigateToPharmacyLocator() {
		...
		CommonUtility.waitForPageLoadNew(driver, pharmacyLocatorHeading, CommonConstants.TIMEOUT_30);
		try {
			Thread.sleep(10000);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
	...
	public ClaimSummaryPage navigateToMedicalClaimsSummary() {
		...
		validateNew(searchMedicalClaims);
	...
	public Boolean tempIdValidation() {
		validateNew(viewIDCard);
		viewIDCard.click();
		if(validateNew(validateLogo)){
			return true;
		}
		return false;
	...
	*/
	...
	public boolean validateAccountHome(){
		boolean flag = false;
		if(validateNew(homePageContent)&&validateNew(planBox)&&validateNew(logOut)&&validateNew(myResourcesContent)&&validateNew(drugLookupBox)&&validateNew(estimateCostsBtn))
			flag = true;
		return flag;
	}
	...
	public BenefitsAndCoveragePage1 navigateDirectToBnCPag() {
