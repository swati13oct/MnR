...
import pages.deprecated.member.bluelayer.AddPlanPopUpPage;
...
public class PlanSummaryPage extends UhcDriver {
	...
	public PlanSummaryPage(WebDriver driver, String category) {
		...
		PageFactory.initElements(driver, this);
		String fileName = CommonConstants.PLAN_SUMMARY_PAGE_DATA;
		CommonUtility.waitForPageLoadNew(driver, planInformationHeading,CommonConstants.TIMEOUT_30);
		CommonUtility.waitForPageLoadNew(driver, claimSectionHeading,CommonConstants.TIMEOUT_30);
		planSummary = CommonUtility.readPageData(fileName,
				CommonConstants.PAGE_OBJECT_DIRECTORY_BLUELAYER_MEMBER);
		openAndValidate();
	}
	...
	public boolean validateClaims() {
		boolean flag = false;
		CommonUtility.waitForPageLoadNew(driver, claimSectionHeading,CommonConstants.TIMEOUT_30);
		if(validateNew(claimSectionHeading)&&validateNew(claimsPlanBox)){
				flag = true;
		}
		return flag;
	}
	...
	public ClaimSummaryPage navigateToDrugClaimsSummary() {
		CommonUtility.waitForPageLoadNew(driver, searchAllClaimsBtn, 25);
