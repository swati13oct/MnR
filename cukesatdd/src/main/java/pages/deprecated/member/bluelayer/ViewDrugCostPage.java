...
public class ViewDrugCostPage extends UhcDriver {
	...
	public ViewDrugCostPage(WebDriver driver) {
		...
		CommonUtility.waitForPageLoadNew(driver, drugCostContent, CommonConstants.TIMEOUT_30);
	...
	public boolean validateViewDrugPage(String dosage) {
		...
		if(validateNew(descriptionTable)&&(drugInfoBox.getText().contains(dosage)) && validateNew(editLink)&&validateNew(editPharmacyLink))
			return true;
		else 
			return false;
	}
