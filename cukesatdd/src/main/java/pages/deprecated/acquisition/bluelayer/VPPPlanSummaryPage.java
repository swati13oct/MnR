...
import pages.deprecated.acquisition.bluelayer.EnrollPlanInfoPage;
...
{
	validateNew(NeedHelp);
	validateNew(MeetAnAgent);
	validateNew(NeedMoreInformation);
	validateNew(NeedStepBack);
	validateNew(GetEmailUpdates);
...

{
	for(int i=1;i<medicalColumn.size();i++){
		System.out.println(medicalColumn.get(i).findElement(By.xpath(".//td[2]/p")).getText());
			validateNew(medicalColumn.get(i).findElement(By.xpath(".//td[2]/p")));
	}
	...
	private WebElement getViewPlanDetailsElement(List<WebElement> maPlanElement2, ElementData elementData,
			String planName) {
		CommonUtility.waitForPageLoadNew(driver,viewDetailsBtnMA,CommonConstants.TIMEOUT_30);
	...
	public void openAndValidate() {
		validateNew(showMaPlans);
		validateNew(showMsPlans);
		validateNew(showPdpPlans);
		validateNew(showSnpPlans);
		vppPlanSummaryJson = formJsonObject(vppPlanSummary);
	}
	...
	public VPPPlanSummaryPage clicksOnIsProviderCoveredA(String planName) {
		...
		waitforElementNew(GetStarted);
		GetStarted.click();

		waitforElementNew( People);

		People.click();

		waitforElementNew(Primary);

		Primary.click();

		waitforElementNew(Physician);

		Physician.click();

		waitforElementNew(Savebtn);

		JavascriptExecutor js = (JavascriptExecutor)driver;
		js.executeScript("arguments[0].click();", Savebtn);

		//Savebtn.click();
		waitforElementNew(Viewsavebtn);
		...
		waitforElementNew(Checkcoverage);
		CommonUtility.waitForPageLoadNew(driver, Checkcoverage, 10);
		Checkcoverage.click();
	...
	public void verifyInactiveCompare3PlansButton(){
		waitforElementNew(comparePDPPlanChkBox);
	...
	public boolean validateVPPPlanSummaryPage(){
		boolean flag = false;
		if(validateNew(viewPlans)&&validateNew(viewPDPPlans)&&validateNew(changeLocationBtn))
			flag = true;
		return flag;
	}
