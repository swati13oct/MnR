...
public class PlanPreviewPage extends GlobalWebElements {
	...
	public void openAndValidate() {
		if (!(currentUrl().contains("uhcmedicaresolutions"))) {
			startNew(UMS_PLANPREVIEW_PAGE_URL);
			validateNew(zipcodetxtbox);
			validateNew(lookuplink);
...
{
		sendkeysNew(zipcodetxtbox, zipcode);
		continuebtn.click();
		Thread.sleep(5000L);
		try {
	...
	public ZipcodeLookupHomePage looksupforZipcodes() {
		lookzip.click();
		CommonUtility.waitForPageLoadNew(driver, zipCodeSearchPopup, CommonConstants.TIMEOUT_30);
