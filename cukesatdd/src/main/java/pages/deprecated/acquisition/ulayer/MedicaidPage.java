...
public class MedicaidPage extends UhcDriver{	
	...
	@Override
		...
		validateNew(medicaiddno);
		validateNew(medicaiddyes);		
	...
	public void entersmedicaidInformation(
		...
		if(medicaidradiooption.equalsIgnoreCase("No")){
			...
			medicaiddyes.click();
			sendkeysNew(medicaidnum,medicaidnumber);
		}				
	}
	...
	public boolean validateMedicadePage(){
		boolean flag = false;
		if(validateNew(medicaidHeader)&&validateNew(medicaidprevious)&&validateNew(medicaidsaveandcontinue)&&validateNew(medicaiddno)&&
		validateNew(medicaiddyes)&&validateNew(medicaidcancleregistration))
			flag = true;
		return flag;
	}
   }
