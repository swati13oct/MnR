...
/*		ElementData elementData = new ElementData("id","step2Heading");
	...
	public boolean validateIntroPage(){
		boolean flag = false;
		if(validateNew(firstNameField) && validateNew(middleInitialField) && validateNew(lastNameField) &&validateNew(claimNumberField) &&
		   validateNew(partAStartDateField)&&validateNew(partBStartDateField)&&validateNew(viewEnrollDisclaimer) && !validateNew(beginOnlineEnrBtn)){
			flag = true;
		}
		return flag;
	}
	...
	public boolean hasEnrolledInLast24hrs() {
		return validateNew(alreadyEnrolledErrorMsg);
