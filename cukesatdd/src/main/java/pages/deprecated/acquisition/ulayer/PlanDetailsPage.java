...
public class PlanDetailsPage extends UhcDriver {
	...
	public PlanDetailsPage(WebDriver driver, String planType) {
		super(driver);
		PageFactory.initElements(driver, this);
		CommonUtility.waitForPageLoadNew(driver, plandetails, CommonConstants.TIMEOUT_30);
	...
	public void openAndValidate() {
		validateNew(medBenefitsTab);
		validateNew(presDrugTab);
    ...
    public boolean validatePlanDetailsPage(){
		...
		if(validateNew(medBenefitsTab)&&validateNew(presDrugTab)&&validateNew(planCostsTab)&&
	...
	public boolean validateCompareBoxMessage() {
		...
		CommonUtility.waitForPageLoadNew(driver, drugBenefitsSection, 20);
		compareChkBox.click();
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
