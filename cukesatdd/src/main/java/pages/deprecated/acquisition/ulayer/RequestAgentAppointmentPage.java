...
public class RequestAgentAppointmentPage extends UhcDriver{
	...
	public boolean validateRequestApptPage(){
		if(validateNew(firstName)&&validateNew(lastName)&&validateNew(address)&&validateNew(city)&&
		validateNew(state)&&validateNew(zip)&&validateNew(phoneField)&&validateNew(requestAppointmentButton))
			return true;
		return false;
	}
