...
public class ReviewAndSubmitPage extends UhcDriver{
	...
	public boolean validateReviewPage(String plantype){
		...
		if(plantype.equals("MA")||plantype.equals("MAPD")){
			if(validateNew(authRepresentOptionagreeBtn1)&&validateNew(authRepresentOptionagreeBtn2)&&validateNew(stmtofUnderstandingAgreeBtn)&&validateNew(stmtofUnderstandingDisagreeBtn)&&
			validateNew(stmtofUnderstandingDisclaimerBtn)&&validateNew(benInfoEditBtn)&&validateNew(sepEditBtn)
			&&validateNew(pcpEditBtn)&&validateNew(ltcEditBtnMA)&&validateNew(esrdEditBtn)&&validateNew(pdcEditBtnMA)&&validateNew(medicaidEditBtn)&&validateNew(ppoEditBtnMA)
			&&validateNew(pedEditBtn)&&validateNew(otherHealthInsEditBtn)&&validateNew(editIntroBtn)&&validateNew(cancelBtnMA)&&validateNew(printBtnMA)&&validateNew(submitBtn))
				flag = true;
		}else if(plantype.equals("PDP")){
			if(validateNew(authRepresentOptionagreeBtn1)&&validateNew(authRepresentOptionagreeBtn2)&&validateNew(stmtofUnderstandingAgreeBtn)&&validateNew(stmtofUnderstandingDisagreeBtn)&&
			validateNew(stmtofUnderstandingDisclaimerBtn)&&validateNew(benInfoEditBtn)&&validateNew(sepEditBtn)&&validateNew(ltcEditBtnPDP)
			&&validateNew(pdcEditBtnPDP)&&validateNew(ppoEditBtnPDP)&&validateNew(pedEditBtn)&&validateNew(editIntroBtn)&&validateNew(cancelBtnPDP)&&validateNew(printBtnPDP)&&validateNew(submitBtn))
				flag = true;
		}else{
