...
public class PrescriptionDrugCoveragePage extends UhcDriver{	
	...
	public void enterspdcInformation(
		...
		if(pdcradiooption.equalsIgnoreCase("No")){
			...
			pdcyes.click();
			sendkeysNew(pdcnameofinsur,pdchealthinsurname);
			sendkeysNew(pdcgroupid,pdcgroupidnumber);
			sendkeysNew(pdcmemberid,pdcmemberidnumber);
		...
		public boolean validatePDCPage(){
			boolean flag = false;
			if(validateNew(pdcHeader)&&validateNew(pdcsaveandcont)&&validateNew(pdccancel)&&validateNew(pdcno)&&validateNew(pdcyes))
				flag = true;
			return flag;
		}
}
