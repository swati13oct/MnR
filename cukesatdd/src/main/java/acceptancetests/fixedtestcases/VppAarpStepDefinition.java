...
System.out.println("planSummaryExpectedJson---->"
	...
	@And("^the user access the enter drug information link for above selected plan section in the AARP site$")
	...
	public void user_selects_enter_drug_info_link(){
		String planName = (String) getLoginScenario().getBean(VPPCommonConstants.PLAN_NAME);
		VPPPlanSummaryPage planSummaryPage = (VPPPlanSummaryPage) getLoginScenario()
				.getBean(PageConstants.VPP_PLAN_SUMMARY_PAGE);
		GetStartedPage getStartedPage = planSummaryPage.clicksOnEnterDrugInformationLink(planName);
		try {
			Thread.sleep(10000);
	...
	@And("^the user selects the type of pharmacy and distance in the AARP site$")
	...
	public void user_validates_the_plansummary_after_drug_info_aarpsite()
	{
		JSONObject planSummaryActualJson = (JSONObject) getLoginScenario()
				.getBean(VPPCommonConstants.VPP_PLAN_SUMMARY_ACTUAL);
		JSONObject planSummaryExpectedJson = (JSONObject) getLoginScenario()
	...
	public void user_views_plan_details_ums() {
		...
		PlanDetailsPage planDetailsPage = (PlanDetailsPage) manageDrugPage
	...
	public void user_views_plandetails_for_above_selected_plan_ums() {
	    ...
	    @And("^the user views plan details for the selected plan in the AARP site$")
	...
	@And("^the user clicks on Enter Provider information link in the AARP site$")
	...
	@Then("^the user validates the plan summary after provider information is added in the AARP site$")
	...
	@When("^the user performs plan search on learnfindplans using following information in the AARP site$")
	...
	public void bacK_to_planSummary() {
		...
		vpplanSummaryPage = vpplanSummaryPage.togglePlanYear(planType);
		if(null!= vpplanSummaryPage){
			getLoginScenario().saveBean(PageConstants.VPP_PLAN_SUMMARY_PAGE,
					vpplanSummaryPage);
		}
		else{
			System.out.println("Switch to current year plans failed");
			Assert.fail("Switch to current year plans failed");
		}
	}
	...
	public void verify_nextyear_enrolllink() {
		...
		enrollFlag = plansummary.verifyandclickenrolllink(plantype);
		Assert.assertTrue(enrollFlag);
	}
