...
public class VppUmsStepDefinition {
	...
	@Before("@fixedTestCaseAEP")
	public void setupforAEP() {
		/* Format(MM-DD-YYYY) AEP Test */
		String date = "11-11-2016";
		CommonUtility.changeMRRestTime(getLoginScenario(), date);
		CommonUtility.changePartDTime(getLoginScenario(), date);
		System.out.println("Changed MRREST and PartD time to:::"+date);
		/*Date change logic ends*/
	}

	
	@After("@fixedTestCaseAEP")
	public void tearDownforAEP() {
		...
		System.out.println("Reset successful on MRREST and PartD time");
	}	
	
	
	
	@Before("@fixedTestCaseNonAEP")
	public void setup() {
		/* Format(MM-DD-YYYY) NON-AEP Test */
		String date = "09-10-2016";
		CommonUtility.changeMRRestTime(getLoginScenario(), date);
		CommonUtility.changePartDTime(getLoginScenario(), date);
		System.out.println("Changed MRREST and PartD time to:::"+date);
		/*Date change logic ends*/
	...
	@When("^the user performs plan search on learnfindplans using following information in the UMS site$")
	...
	@And("^the user selects the plan in the UMS site$")
	...
	@And("^the user selects the enter drug information link for the selected plan in the UMS site$")
	...
	@And("^the user search for the drug in the UMS site$")
	...
	@And("^the user selects the drug from the dropdown in the UMS site$")
	...
	@And("^the user selects low cost options for the selected drug in the UMS site$")
	...
	public void user_performs_paharmacySearch_ums() {
		...
		if (selectPharmacyPage != null) {
			...
			String directory = CommonConstants.ACQUISITION_EXPECTED_DIRECTORY
					...
					+ File.separator + zipcode + File.separator + county
					+ File.separator + planName + File.separator + miles + File.separator;
	...
	@And("^the user search for pharmacies in the UMS site$")
	...
	public void user_selects_pharmacyType_and_distance_ums(DataTable pharmacyAttributes) {
		...
		String directory = CommonConstants.ACQUISITION_EXPECTED_DIRECTORY
				...
				+ File.separator + DceCommonConstants.SELECT_PHARMACY_FLOW
				+ File.separator + zipcode + File.separator + county
				+ File.separator + planName + File.separator + distance + File.separator;
	...
	@And("^the user selects the type of pharmacy and distance in the UMS site$")
	...
	public void user_selects_pharmacy_ums(DataTable pharmacyAttributes) {
		...
		if (manageDrugPage != null) {
			...
			String directory = CommonConstants.ACQUISITION_EXPECTED_DIRECTORY
					...
					+ File.separator + planName + File.separator + drugWithDosage + File.separator;
	...
	@And("^the user selects the below pharmacy from the list of pharmacies in the UMS site$")
	...
	public void user_views_plandetails_selected_plan_ums() {
	 ...
	 @Then("^the user views plan details for the selected plan in the UMS site$")
	...
	public void zipcode_details_in_ums_site_aep(DataTable givenAttributes) {
		...
		if (plansummaryPage != null) {
			...
			String directory = CommonConstants.ACQUISITION_EXPECTED_DIRECTORY
					...
					+ File.separator + VPPCommonConstants.VPP_PLAN_FLOW_NAME
					+ File.separator + zipcode + File.separator + county
					+ File.separator + CommonConstants.NEXT_YEAR + File.separator;
			JSONObject planSummaryExpectedJson = MRScenario.readExpectedJson(
					fileName, directory);
			getLoginScenario().saveBean(
					VPPCommonConstants.VPP_PLAN_SUMMARY_EXPECTED,
...
*/
		...
		String directory = CommonConstants.ACQUISITION_EXPECTED_DIRECTORY
				...
				+ zipcode + File.separator + county + File.separator + CommonConstants.NEXT_YEAR + File.separator;
		JSONObject planSummaryExpectedJson = MRScenario.readExpectedJson(
				fileName, directory);
	...
	public void bacK_to_planSummaryPage() {
		...
		PlanDetailsPage vppPlanDetailsPage = (PlanDetailsPage) getLoginScenario()
				.getBean(PageConstants.VPP_PLAN_DETAILS_PAGE);
		VPPPlanSummaryPage vppPlanSummaryPage = vppPlanDetailsPage
				.backtoPlanSummaryPage(planType);
		
		vppPlanSummaryPage = vppPlanSummaryPage.togglePlanYear(planType);
		
		getLoginScenario().saveBean(PageConstants.VPP_PLAN_SUMMARY_PAGE,
				vppPlanSummaryPage);
	
	}
	@And("^the user validate pdf links on the UMS site$")
