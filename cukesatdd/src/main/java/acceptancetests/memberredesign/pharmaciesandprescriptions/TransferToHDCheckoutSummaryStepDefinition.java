...
public class TransferToHDCheckoutSummaryStepDefinition {
	 ...
	 * public void user_will_be_brought_to_the_page_for_that_medication(String
	 * expectedPage) throws Throwable { CheckOutSummaryPage checkoutSumaryPg = new
	 * CheckOutSummaryPage(null); PharmaciesAndPrescriptionsPage pnpPg =
	 * (PharmaciesAndPrescriptionsPage) getLoginScenario()
	 * .getBean(PharmaciesAndPrescriptionsCommonConstants.
	 * PHARMACIES_AND_PRESCRIPTIONS_PAGE); checkoutSumaryPg =
	 * pnpPg.navigateToCheckOutSummaryPage();
	 * getLoginScenario().saveBean(PageConstants.CHECKOUT_SUMMARY_PAGE,
	 * checkoutSumaryPg); Assert.assertTrue("PROBLEM - " + expectedPage +
	 * " Page not available",
	 ...
	 * ) public void
	 * user_fetches_medication_informations_and_clicks_on_Transfer_To_HD_call_to_action_button
	 ...
	 * pnpPg.getListOfIndexForRetailTransferToHDOnMyMed(); int countOfPage =
	 * Integer.parseInt(pnpPg.getCountOfMyMedPage()); for (int i = 0; i <
	 * countOfPage; i++) { if (indexOfRenewMedication.size() == 0 && i !=
	 * countOfPage - 1) { pnpPg.clickOnNextPageArrow(); indexOfRenewMedication =
	 * pnpPg.getListOfIndexForRetailTransferToHDOnMyMed(); } }
	 * listOfMedicationDetail = pnpPg.fetchesMedicationInformationFrTransferToHD();
	 * int medicationToBeClicked = (int)
