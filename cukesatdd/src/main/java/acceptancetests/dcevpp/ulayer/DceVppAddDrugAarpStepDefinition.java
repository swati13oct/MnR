...
public class DceVppAddDrugAarpStepDefinition {
	...
	@And("^user selects following drug in AARP site$")
	...
	@Then("^user is on the AARP medicare site home page$")
	public void validate_dce_from_homepage_enterzipcode_widget()
	{
		WebDriver wd = getLoginScenario().getWebDriver();
	...
	@Then("^user performs drug search using the following details in AARP site$")
	public void validate_dce_from_landing_page_enterzipcode_widget(DataTable givenAttributes)
	{
		...
		getLoginScenario().saveBean(DceCommonConstants.COUNTY_NAME, county);
		AcquisitionHomePage acqusitionHomePage =(AcquisitionHomePage) getLoginScenario().getBean(PageConstants.ACQUISITION_HOME_PAGE);
		VPPPlanSummaryPage vppSummary = (VPPPlanSummaryPage)acqusitionHomePage.enterZipcode(zipCode,county,planYear);
		GetStartedPage getStartedPage = (GetStartedPage)vppSummary.navigateToSummaryPage(planType);
		AddDrugPage addDrugPage = getStartedPage.navigateToDCE();
		if (addDrugPage != null) {
			getLoginScenario().saveBean(PageConstants.ADD_DRUG_PAGE,
					addDrugPage);
