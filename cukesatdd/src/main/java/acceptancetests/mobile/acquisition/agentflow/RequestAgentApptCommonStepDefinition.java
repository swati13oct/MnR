...
public class RequestAgentApptCommonStepDefinition {
	...
	public void navigates_request_more_help_information1() {
		AcquisitionHomePageMobile aquisitionhomepage = (AcquisitionHomePageMobile) getLoginScenario()
				.getBean(PageConstants.ACQUISITION_HOME_PAGE);
		RequestHelpAndInformationPageMobile requestHelpAndInformationPage = aquisitionhomepage
				.navigateToMaMoreHelpAndInfo();

		if (requestHelpAndInformationPage != null) {
			getLoginScenario().saveBean(PageConstants.REQUEST_MORE_HELP_INFORMATION_PAGE,
					requestHelpAndInformationPage);
			Assertion.assertTrue(true);
	...
	public void request_appointment() {
		...
		RequestAgentAppointmentPageMobile requestAgentAppointmentPage = requestHelpAndInformationPage
				.navigateToAgentAppointmentRequest();
