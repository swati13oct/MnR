...
import pages.deprecated.member.ulayer.AddDrugPage;
import pages.deprecated.member.ulayer.DrugDosagePage;
import pages.deprecated.member.ulayer.EstimateYourDrugCostPage;
import pages.deprecated.member.ulayer.LoginPage;
import pages.deprecated.member.ulayer.LowCostOptPage;
import pages.deprecated.member.ulayer.ManageDrugPage;
import pages.deprecated.member.ulayer.SelectPharmacyPage;
import pages.deprecated.member.ulayer.ViewDrugCostPage;
...
public class DrugLookupAarpStepDefinition {
	...
	public void selectPlan(DataTable memberAttributes){
		...
		ManageDrugPage manageDrugPage = (ManageDrugPage) getLoginScenario()
				.getBean(PageConstants.MANAGE_DRUG_PAGE_VBF);
			...
			if (lowCostOptionsPage != null) {
				...
				if(lowCostOptionsPage.validateLowCostSection(drugDosage)){
					...
					if (!drugType.equalsIgnoreCase("null") && null != drugType) {
							...
							if (manageDrugPage != null) {
								getLoginScenario().saveBean(PageConstants.MANAGE_DRUG_PAGE_VBF,
										manageDrugPage);
	...
	public void user_selects_branded_generic_drug(
		...
		if (!drugType.equalsIgnoreCase("null") && null != drugType) {
			if (getLoginScenario().getBean(PageConstants.LOW_COST_OPT_PAGE) != null){
				...
				ManageDrugPage manageDrug = (ManageDrugPage) getLoginScenario().getBean(PageConstants.MANAGE_DRUG_PAGE_VBF);
				manageDrug = manageDrug.selectDrugType(drugType);
				 getLoginScenario()
					.saveBean(PageConstants.MANAGE_DRUG_PAGE_VBF, manageDrug);
				Assert.assertTrue(true);
			}
	...
	public void user_will_validate_the_Drug_cost_information(DataTable pharmacyNameAttribute) {
		String dosage = pharmacyNameAttribute.getGherkinRows().get(0)
				.getCells().get(1);
