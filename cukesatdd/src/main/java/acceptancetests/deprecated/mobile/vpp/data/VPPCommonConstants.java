/**
 * 
 */
package acceptancetests.deprecated.mobile.vpp.data;

/**
 * @author pjaising
 *
 */
public class VPPCommonConstants {

	public static final String ZIPCODE = "zipcode";
	
	public static final String COUNTY = "county";

	public static final String VPP_PLAN_FLOW_NAME = "vppplansummary";

	public static final String VPP_PLAN_SUMMARY_EXPECTED = "planSummaryExpectedJson";

	public static final String VPP_PLAN_SUMMARY_ACTUAL = "planSummaryActualJson";

	public static final String PLAN_NAME = "planName";

	public static final String PLAN_TYPE = "planType";

	public static final String VPP_PLAN_DETAIL_ACTUAL = "planDetailsActualJson";

	public static final String VPP_PLAN_DETAILS_FLOW_NAME = "vppplandetails";

	public static final String VPP_PLAN_DETAIL_EXPECTED = "planDetailsExpectedJson";

	public static final String ENROLL_IN_PLAN_FLOW_NAME = "enrollinplan";

	public static final String SWITCHED_TO_GENEIC = "swictedtogeneric";
	
	public static final String YEAR = "year";

	public static final String TOGGLEPLANFLAG = "togglePlanFlag";
	
	public static final String VPP_PLAN_DOC_PDF_EXPECTED = "planDocsPDFExpectedJson";

	public static final String FIRST_NAME = "firstname";

	public static final String LAST_NAME = "lastname";

	public static final String EMAIL_ADDRESS = "emailaddress";

}
